...
<script>
// ==社交接口=======查询 ======重置密码  ===========封号=======删除======   解封====
import { getSocialMember, resetMemberSocialPassword, banMemberSocial, deleteMemberSocial, unBanMemberSocial } from '@/api/members'
export default {
  data() {
    return {
      limitId: '',
  ...
  methods: {
    ...
    resetPass(index, row) {
      console.log(index)
      console.log(row)
      this.$confirm('（重置后密码统一设置为：w1234567）', '确定重置该用户密码?', {
    ...
    // 点击查看详情
    view(row) {
      this.$router.push(`/staff/detail/${row.mId}`)
